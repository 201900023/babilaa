...
const PostCardFooter = ({ post }: PostCardFooterProps) => {
  ...
  const { data: comments, isSuccess: isCommentsSuccess } = usePostCommentsQuery(
    post.id
  );
  ...
  return (
    <>
      <Dialog
        className={
          "bg-black bg-opacity-50 w-full h-full fixed top-0 left-0 z-50 flex items-end justify-center "
        }
        open={isOpen}
        ...
        <Dialog.Panel>
          <div className="bg-purple-100  w-screen rounded-t-xl h-[600px] grid grid-rows-[40px_460px_100px]  py-3  overflow-hidden ">
            <header className=" rounded-t-xl bg-transparent flex items-center  flex-col ">
              <p className="font-semibold text-purple-0">
                {comments?.length} Comments
              </p>

              <div className="border-purple-0 border-t-2 w-[90%] mt-3">
                &nbsp;
              </div>
            </header>
      ...
      <div className="items-center flex justify-around gap-2 py-3   text-purple-0 ">
        ...
        >
          <FaRegCommentDots
            size={21}
            className={clsx(post.sharedByMe && "fill-green-600")}
          />
